...
from .utils.resnet import resnet18, resnet34, resnet50
...
class Algorithm(AbstractAlgorithm):
    ...
    def update(self, x, y):
        ...
        with autocast(enabled=self.algorithm_cfg.mixedprec):
            # TODO: implement your training routine here
            logits = self.model(x)
    ...
    def _init_model(self):
        if self.algorithm_cfg.backbone == 'resnet18':
            return resnet18(pretrained=False,
                            num_classes=self.num_classes,
                            with_permute_adain=True,
                            p_adain=self.algorithm_cfg.padain)
        elif self.algorithm_cfg.backbone == 'resnet18':
            return resnet34(pretrained=False,
                            num_classes=self.num_classes,
                            with_permute_adain=True,
                            p_adain=self.algorithm_cfg.padain)
        elif self.algorithm_cfg.backbone == 'resnet50':
