...
def random_sample(prob, sampling_num):
    ...
    return torch.multinomial((prob.reshape(batch_size * channels, -1) + 1e-8), sampling_num, replacement=True)
...
class Info_Dropout(nn.Module):
    ...
    def forward(self, x_old, x):
        ...
        with torch.no_grad():
            ...
            distance = (distance**2).reshape(-1, self.indim, h_dis, w_dis).sum(dim=1).reshape(batch_size, -1, h_dis, w_dis)
            ...
            random_mask = torch.ones((batch_size * channels, h * w)).to(torch.cuda.current_device())
            random_mask[torch.arange(batch_size * channels).to(torch.cuda.current_device()).reshape(-1, 1), random_choice] = 0

        return x * random_mask.reshape(x.shape)
