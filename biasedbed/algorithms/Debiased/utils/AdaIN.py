...
vgg = nn.Sequential(
    ...
    nn.Conv2d(64, 64, (3, 3)),
    nn.ReLU(),  # relu1-2
    nn.MaxPool2d(kernel_size=(2, 2), stride=(2, 2), padding=(0, 0), return_indices=False, ceil_mode=True),
    nn.ReflectionPad2d((1, 1, 1, 1)),
...
class StyleTransfer:
    def __init__(self, rank, decoder_ckpt='./decoder.pth', vgg_ckpt='./vgg_normalised.pth'):
        self.rank = rank
        self.decoder = decoder
        self.decoder.eval()
        self.decoder.load_state_dict(torch.load(decoder_ckpt))
        self.decoder.to(self.rank)

        self.vgg = vgg
        self.vgg.eval()
        self.vgg.load_state_dict(torch.load(vgg_ckpt))
        self.vgg = nn.Sequential(*list(self.vgg.children())[:31])
        self.vgg.to(self.rank)
    ...
    def __call__(self, image, label, alpha, replace=True, label_mix_alpha=0):
        ...
        style = image[random_index]
        label_style = label.detach()[random_index]
        with torch.no_grad():
            stylized_image = self.style_transfer(content, style, alpha)

        if replace:
            return stylized_image, (label, label_style, torch.ones(n).to(self.rank) * label_mix_alpha)
        else:
            label1 = torch.cat([label, label])
            label2 = torch.cat([label_style, label_style])
            label_weight = torch.cat([torch.zeros(n), torch.ones(n) * label_mix_alpha]).to(self.rank)
